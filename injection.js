function HahaInjection() {
    ...
    /**
      ...
      */
    this.analizeTrendPool = trendPool => {
      const winners = trendPool.result.split('').reduce((acc, cur, index) => {
        if (cur == '1') {
          const n = this.catMap[(index + 1).toString()];
          acc.push(n);
        }
        return acc;
      }, []);
      const cards = trendPool.cards.split('#');
    ...
    this.handleTrendMessage = msg => {
      ...
      const currentPool = this.pools[this.pools.length - 1];
      this.currentPool = {
        table: this.tableId,
      ...
      const tablesStr = window.localStorage.getItem('@tables');
      if(tablesStr){
    ...
    this.handlePoolMessage = msg => {
      ...
      this.currentPool.volume = this.mapVolume(
        msg.content.cat,
        msg.content.amount,
      );
    };
    ...
    this.handleCommitSucceed = msg => {
      let pool = this.analizeTrendPool(msg.content);
    ...
    (this.handleMessage = evt => {
      ...
      (this.toCsv = () => {
        let data = window.localStorage.getItem('@table_' + this.tableId);
        if (data) {
        ...
        const csvData = [];
        csvData.push(
          'table;round;roundId;winner;player cards;banker cards;BANKER;PLAYER;TIE;BANKER_PAIR;PLAYER_PAIR;BIG;SMALL;BANKER_N8;PLAYER_N8;BANKER_N9;PLAYER_N9;SUPER',
        );
        data.forEach(item => {
          const r = [];
          r.push(item.table);
          ...
          const cats = Object.keys(this.catMap).map(key => this.catMap[key]);
          for (let index = 0; index < cats.length; index++) {
            const catName = cats[index];
            if (
              typeof item.volume !== 'undefined' &&
              typeof item.volume[catName] !== 'undefined'
            ) {
        ...
        displayBox.style = 'padding: 30px;pos';
        displayBox.innerHTML = csvData.join('<br/>');
        document.body.appendChild(displayBox);
      });
  }
  setTimeout(() => {
    window.myHahaInjection = new HahaInjection();
    window.netutils.webSocket.onmessage = function(evt) {
      window.netutils.onMessage(evt);
      window.myHahaInjection.handleMessage(evt);
    };
  }, 5000);
